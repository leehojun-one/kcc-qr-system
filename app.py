...
def status_bar(status):
    """접수→접수확인→생산중→배송중→현장도착→완료 진행 표시 문자열."""
    flow = ["접수", "접수확인", "생산중", "배송중", "현장도착", "완료"]
    cur = flow.index(status) if status in flow else 0
    parts = []
...
if role.startswith("①"):
    ...
    with tab2:
        ...
        # ── 내가 등록한 당일사고 진행현황 (모니터링 + 현장도착 확인) ──
        st.divider()
        st.markdown("**📋 당일사고 진행현황**")
        my_inc = [i for i in db.all_incidents()
                  if i["issue_type"] == "당일사고" and i["status"] != "완료"]
        if not my_inc:
            st.caption("진행 중인 당일사고가 없습니다.")
        for i in my_inc:
            s = db.get_site(i["quote_no"])
            with st.container(border=True):
                st.write(f'**{s["address"].split("(")[0].strip()}** · {i["window_location"]} · {i["vendor"]}')
                st.markdown(status_bar(i["status"]))
                if i.get("vendor_schedule"):
                    st.caption(f'🗓️ 처리예정일: {i["vendor_schedule"]}')
                if i.get("done_photo"):
                    st.caption(f'🔧 처리방법: {i["done_photo"]}')
                if i["status"] == "배송중":
                    if st.button("📦 현장도착 확인", key=f"arr{i['id']}", type="primary"):
                        db.set_incident_status(i["id"], "현장도착"); st.rerun()
...
# ═══════════════════════ ② 파트너사 앱 ═══════════════════════
    ...
    if scan_serial and scan_serial in qr_map:
        serial = scan_serial
        st.info(f"📷 보증 QR 스캔으로 열림 — {serial}")
    else:
        st.warning("이 화면은 거실 메인창의 **보증 QR을 스캔**해야 열립니다. "
                   "(폰 카메라로 QR을 찍으면 본인 세대로 바로 연결돼요)")
        with st.expander("🔧 데모용 — QR 스캔 시뮬레이션 (시연 전용)"):
            serial = st.selectbox("스캔할 세대(보증 QR)", list(qr_map.keys()))
            if not st.checkbox("이 QR을 스캔한 것으로 진행"):
                st.stop()
    qno = qr_map[serial]
        ...
        with st.container(border=True):
            ...
            if is_pre:
                if st.button("📤 가공처에 지시 발송", key=f"v{i['id']}"):
                    ...
                    db.log_notifs([log])
                    db.set_incident_status(i["id"], "접수확인")
                    st.success(f'가공처({site["vendor"]})에 지시 발송 — {log["content"]}')

    st.divider()

    # ── 완료확인 대기 (현장도착 → 완료) ──
    arrived = [i for i in incidents if i["status"] == "현장도착"]
    st.subheader(f"✅ 완료확인 대기 ({len(arrived)}건)")
    if not arrived:
        st.caption("완료확인 대기 중인 건이 없습니다.")
    for i in arrived:
        site = db.get_site(i["quote_no"])
        with st.container(border=True):
            st.write(f'**{site["address"].split("(")[0].strip()}** · {i["window_location"]} · {i["vendor"]}')
            if i.get("done_photo"):
                st.caption(f'🔧 처리방법: {i["done_photo"]}')
            if st.button("✅ 완료 확인", key=f"close{i['id']}", type="primary"):
                db.set_incident_status(i["id"], "완료"); st.rerun()
...
# ═══════════════════════ ⑥ 가공처 ═══════════════════════
    ...
    st.header("⑥ 가공처")
    vendor = st.selectbox("가공처 선택", sqlite_db.VENDORS)
    st.caption("접수된 사고를 확인 → 접수확인/생산/배송 진행 + 처리예정일·처리방법 입력. (완료확인은 관제센터)")

    incidents = [i for i in db.all_incidents()
                 if vendor and vendor in str(i.get("vendor") or "")]
    active = [i for i in incidents if i["status"] != "완료"]
    new_cnt = len([i for i in incidents if i["status"] == "접수"])
    if new_cnt:
        st.error(f"🔔 새 사고 {new_cnt}건 — 접수확인이 필요합니다")
    if not active:
        st.info("처리할 사고가 없습니다.")
    ...
    for i in active:
        ...
        with st.container(border=True):
            ...
            st.caption(f'사유: {i["fault_provisional"]} · 신고: {i["reporter"]} · {i["created_at"]}')
            if i.get("note"):
                st.info(f'📝 현장 특이사항: {i["note"]}')
            st.markdown(status_bar(i["status"]))

            # 상태 진행 버튼
            b1, b2, b3 = st.columns(3)
            if i["status"] == "접수":
                if b1.button("✅ 접수확인", key=f"ack{i['id']}", type="primary"):
                    db.set_incident_status(i["id"], "접수확인"); st.rerun()
            elif i["status"] == "접수확인":
                if b1.button("🏭 생산 시작", key=f"prod{i['id']}", type="primary"):
                    db.set_incident_status(i["id"], "생산중"); st.rerun()
            elif i["status"] == "생산중":
                if b1.button("🚚 배송 시작", key=f"ship{i['id']}", type="primary"):
                    db.set_incident_status(i["id"], "배송중"); st.rerun()
            elif i["status"] == "배송중":
                b1.success("배송중 — 시공팀의 현장도착 확인 대기")
            elif i["status"] == "현장도착":
                b1.info("현장도착 — 관제센터 완료확인 대기")

            # 처리예정일(달력) + 처리방법
            cur_sched = i.get("vendor_schedule", "")
            try:
                dval = datetime.strptime(cur_sched, "%Y-%m-%d").date() if cur_sched else date.today()
            except Exception:
                dval = date.today()
            sched = b2.date_input("처리예정일", value=dval, key=f"sch{i['id']}")
            method = st.text_area("처리방법 (상세)", value=i.get("done_photo", ""),
                                  key=f"mth{i['id']}", placeholder="예: 24T 로이유리 재제작, 7/3 오전 용차 발송 예정 / 용차기사 010-...")
            if st.button("💾 처리예정일·처리방법 저장", key=f"save{i['id']}"):
                db.update_incident_field(i["id"], "vendor_schedule", str(sched))
                db.update_incident_field(i["id"], "done_photo", method)
...
# ═══════════════════════ ⑦ 영업사원 ═══════════════════════
    ...
    open_cnt = len([i for i in my_inc if i["status"] != "완료"])
    c1, c2 = st.columns(2)
    ...
    if my_inc:
        ...
        st.dataframe([{
            ...
            "상태": i["status"],
            "처리예정일": i.get("vendor_schedule", ""),
            "처리방법": i.get("done_photo", ""),
            "특이사항": i.get("note", ""),
            "접수": i["created_at"],
